...
export default function FintracRevamp() {
  ...
  // Advanced data nesting grouping into: Month -> Week (Mon-Sun) -> Day -> Entries
  const organizedData = useMemo(() => {
    ...
    entries.forEach(e => {
      ...
      // 1. Determine Month Layer
      const mKey = date.toLocaleString('default', { month: 'long' }).toUpperCase();

      // 2. Determine Calendar Week Layer (Monday to Sunday)
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-8 px-4 border-b border-white/5 pb-4">
          <div>
            ...
            <p className="text-[9px] font-mono font-bold text-[#bfff00] mt-1 tracking-wider flex items-center gap-1.5">
              <Clock size={10} /> {liveTime || "LOADING SYSTEM TIME..."}
            </p>
          </div>
        ...
        {/* Render View: Month -> Week -> Day Container -> Logs */}
        <div className="space-y-16 pb-20">
          {Object.keys(organizedData).map(mKey => (
            <div key={mKey} className="bg-white/[0.01] border border-white/5 rounded-[3rem] p-6 md:p-10 space-y-8 shadow-inner">
              ...
              <div className="grid grid-cols-1 gap-6">
                {Object.keys(organizedData[mKey].weeks).sort().reverse().map(wKey => {
                  ...
                  return (
                    <div key={wKey} className={`bg-white/[0.02] border border-white/5 rounded-[2.5rem] overflow-hidden transition-all duration-300 ${isExpanded ? 'ring-1 ring-[#bfff00]/10 bg-white/[0.03]' : ''}`}>

                      {/* Week Accordion Control Trigger */}
                      <button
                        onClick={() => toggleWeek(weekId)}
                        className="w-full flex justify-between items-center p-6 hover:bg-white/[0.01] transition-colors text-left"
                      >
                        ...
                        <div className="text-right">
                          <p className="text-[7px] opacity-20 font-black uppercase tracking-widest mb-0.5">Week Totals</p>
                      ...
                      {/* Dropdown Content: Sorted Days Inside Week */}
                      {isExpanded && (
                        <div className="px-6 pb-6 pt-2 space-y-6 bg-black/20 border-t border-white/[0.02]">
                          {Object.keys(organizedData[mKey].weeks[wKey].days).sort((a, b) => new Date(b).getTime() - new Date(a).getTime()).map(dKey => (
                            <div key={dKey} className="space-y-3">
                              ...
                              {/* Day Label */}
                              <div className="flex items-center gap-3 opacity-40 pl-2">
                                <span className="w-1.5 h-1.5 rounded-full bg-[#bfff00]"></span>
                                <p className="text-[9px] font-black tracking-wider uppercase">{dKey}</p>
                              </div>

                              {/* Transaction Row Generation */}
                              {organizedData[mKey].weeks[wKey].days[dKey].map((item: any) => {
                                const itemTime = new Date(item.created_at).toLocaleTimeString('default', {
                                  hour: '2-digit',
                                  minute: '2-digit',
                                  hour12: false
                                });
                                    ...
                                    <div className="flex items-center gap-5">
                                      ...
                                      <div>
                                        ...
                                        <div className="flex items-center gap-2 mt-1">
                                          <p className="text-[7px] font-black opacity-20 uppercase tracking-wider">{item.category}</p>
                                          <span className="text-[7px] opacity-10">•</span>
                                          <p className="text-[7px] font-mono font-bold text-[#bfff00]/40 tracking-tight">{itemTime}</p>
                                        </div>
                                      </div>
                                    </div>
